...
on its own. Frontmatter is `description` / `argument-hint` / `allowed-tools` / `model`.
_Avoid_: mode, slash-skill

**Agent**: A single-job worker, one file at `agents/<name>.md`, dispatched through the Agent tool
(named Task in older versions) with its own model and system prompt (reviewer, backend, product). A
command or Claude dispatches it; a user does not slash-invoke it. Frontmatter is `name` /
`description` (with dispatch examples) / `model` / `effort` / `tools` / `skills`, validated by
scripts/check-agents.sh against the fields the current schema accepts.
