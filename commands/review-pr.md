...
3. **Check for over-engineering.** Run `/ponytail-review` on the diff: is there a simpler path, a
   reused helper, a stdlib or native feature, or a one-liner that the PR missed.
4. **Verify.** Dispatch `verifier` to confirm each finding is real and reproducible; drop the false
   positives. A finding with no reproducible trigger is labeled plausible, not confirmed.
5. **Report.** Post or return a review: the confirmed findings ordered by severity, each with a
