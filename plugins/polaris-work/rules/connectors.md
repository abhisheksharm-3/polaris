...
<!-- Canonical copy. Mirrored to plugins/polaris-work/rules/connectors.md, because both plugins -->
<!-- read connectors and ship independently, so neither can reference the other's files. The test -->
<!-- suite asserts the two are byte-identical: edit this one, then copy it across. -->

<!-- Read on demand by /catchup here, and by /journal and /sweep in the polaris-work plugin. -->
<!-- How to read a bounded window of work out of -->
...
`gh` is deterministic, so polaris-work's `scripts/journal-facts.sh` runs it and prints the results as facts. The
